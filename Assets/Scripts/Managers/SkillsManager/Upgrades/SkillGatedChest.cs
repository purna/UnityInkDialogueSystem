...
public class SkillGatedChest : MonoBehaviour
{
    ...
    private bool CanOpen()
    {
        ...
        if (!string.IsNullOrEmpty(requiredSkillName) && SkillsTreeManager.Instance != null)
        {
            var skill = SkillsTreeManager.Instance.SkillTreeContainer?.GetSkillByName(requiredSkillName);
    ...
    private void OpenChest()
    {
        ...
        // Grant skill points
        if (skillPointReward > 0 && SkillsTreeManager.Instance != null)
        {
            SkillsTreeManager.Instance.AddSkillPoints(skillPointReward);
        }
